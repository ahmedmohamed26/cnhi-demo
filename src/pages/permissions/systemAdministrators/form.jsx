...
export default function SystemAdminForm({ mapDrawerOpen, onSubmit, users }) {
  ...
  const usersOptions = users.data.items?.map((el) => {
    return {
      label: (i18n.language == "ar" ? el.fullName : record.fullNameEn)?.split(
        "@"
      )[0],
      value: el.userId,
    };
  ...
  function removeSelectedPermissions(allPermissions, selectedPermissions) {
    ...
    return allOfPermissions.filter(
      (element) => !selectedPermissions.includes(element)
    );
  }

  const deletedPermissions = removeSelectedPermissions(
    allPermissions,
    selectedPermissions
  );
                ...
                <Row className="my-4">
                  ...
                  <Col span={16}>
                    <div className="flex gap-8 justify-end">
                      {allPermissionTypes.map((permissionType) => {
                        const isDisabled =
                          !organizationPermissionsMap[category]?.[
                            permissionType
                          ];
                        return (
                          <Checkbox
